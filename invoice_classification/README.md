...
3. **Delivery**: Pass → file into `INTEGRATED/YYYY/MM/`, write `EXTRACTED/<name>.json` (carries `md5` and, when held, a human-readable `validation.reason`), POST to Odoo with the Drive `document_url` and the document's `atcud` (the QR's H field — Odoo's Monthly Reconciliation joins on it exactly); the webhook's `products_unknown` reply lands in the artifact for the QA_Faturas badge. Fail → JSON written, held as `needs_review` for second approval in QA_Faturas; no-QR holds get the extracted date prefilled into the filename. Transient errors retry on later runs
